  ...
  return (
    <section
      className={`absolute bottom-0 left-0 flex flex-col items-center gap-6 bg-primaryColor max-md:-z-10 max-md:w-full max-md:py-4 md:static md:h-full md:flex-row ${
        toggleMenu
          ? 'transition-all duration-300 ease-in-out max-md:translate-y-[99%] max-md:shadow-md'
          : 'transition-all duration-300 ease-in-out max-md:translate-y-[-100%]'
      }`}
    >
      {navLinks.slice(0, -1).map((link) => (
        ...
        >
          {link.name}
          <span className="bg-blue-500 absolute inset-x-0 bottom-0 h-1 scale-x-0 transform transition-transform duration-300 group-hover:scale-x-100 group-hover:bg-shockingPink" />
        </Link>
      ))}
      <a
        target="_blank"
